...
function Mypage({ user, onLogout}) {
  ...
  // 마이페이지 선택된 메뉴 상태 관리 (기본값: 회원정보)
  const [selectedMenu, setSelectedMenu] = useState("회원 정보");

  // 왼쪽 사이드바에 표시될 목록
  const navigationItems = [
    { label: "회원 정보", active: true },
    { label: "회원 정보 수정", active: false },
    ...
    <div className={styles.container}>
      ...
      <div className={styles.contentWrapper}>
          ...
          <div className={styles.sidebarContent}>
            ...
            <div className={styles.navList}>
              {navigationItems.map((item, index) => (
                <button
                  key={index}
                  className={`${styles.navItem} ${selectedMenu === item.label ? styles.active : ""}`}
        ...
        <div className={styles.mainContent}>
          ...
          {/* 회원 정보 수정 */}
          {selectedMenu === "회원 정보 수정" && (
            <MemInfoEdit user={user} />
          )}
        </div>
      </div>
